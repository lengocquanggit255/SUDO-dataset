...
class SVMComparativeModel:
    def __init__(self, aspect, model_path=None):
        ...
        if model_path is None:
            model_path = f"temp/fasttext-svm/svm_checkpoint/{aspect}_best_model.pkl"
        try:
...
class OverallMode(nn.Module):
    def __init__(self, aspect, slac_id, device):
        ...
        # Load SVM model for comparative classification
    ...
    def forward(self, review_1, review_2):
        ...
        try:
            ...
            print(f"SVM prediction failed: {e}")
            # Fallback to no comparison if SVM fails
            return 2
